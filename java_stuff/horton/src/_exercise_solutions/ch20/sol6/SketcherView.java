...
public class SketcherView extends JComponent implements Observer {
  public SketcherView(Sketcher theApp) {
    ...
    JMenuItem scaleItem = elementPopup.add(new JMenuItem("Scale"));      // Scale menu item
  ...
  class MouseHandler extends MouseInputAdapter {
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      switch(mode){
       ...
       case ROTATE:
         // Rotating an element
         if(buttonState == MouseEvent.BUTTON1 && selectedElement != null && !(selectedElement instanceof Element.Text)) {
         ...
         case SCALE:
         if(buttonState == MouseEvent.BUTTON1 && selectedElement != null) {
            selectedElement.scale(last.x-start.x, last.y-start.y);      // Scale it
            repaint();
            start = last;                                               // Make start current point
        }
        break;
